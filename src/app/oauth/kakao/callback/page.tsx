...
const KAKAO_REDIRECT_URI = `${typeof window !== "undefined" ? window.location.origin : "http://localhost:3000"}/oauth/kakao/callback`;
...
export default function KakaoCallbackPage() {
  ...
  useEffect(() => {
    ...
    const handleOAuth = async () => {
      try {
        const res = await api.post("api/v1/auth/oauth/kakao", {
          json: { code, redirect_uri: KAKAO_REDIRECT_URI },
        ...
        if (err && typeof err === "object" && "response" in err) {
          try {
            ...
            setError(body.error?.message || body.detail || "카카오 로그인에 실패했습니다.");
          } catch {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="bg-white rounded-2xl p-10 border border-gray-200 w-[440px] shadow-sm text-center">
        ...
        {status === "processing" && (
          <>
            <div className="w-12 h-12 mx-auto mb-5 border-2 border-yellow-400 border-t-transparent rounded-full animate-spin" />
            <h1 className="text-lg font-bold text-gray-900 mb-2">카카오 로그인 처리 중...</h1>
        ...
        {status === "signup_required" && signupData && (
          <>
            <div className="w-16 h-16 mx-auto mb-5 rounded-full bg-yellow-100 flex items-center justify-center text-3xl">
              💬
            </div>
            <h1 className="text-lg font-bold text-gray-900 mb-2">추가 정보를 입력해 주세요</h1>
            <p className="text-sm text-gray-500 mb-6">카카오 계정으로 처음 로그인합니다.</p>
